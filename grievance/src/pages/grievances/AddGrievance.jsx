...
// Student-only: submit a new grievance → POST api/grievance
// Matches backend Grievances model:
//   { GrievanceId, RaisedBy (userId), CategoryId, Title, Description, StatusId, PriorityId }
export default function AddGrievance({ user }) {
  ...
  const submit = (e) => {
    ...
    // Payload matching backend Grievances model
    // const payload = {
    //   raisedBy:    user?.userId,        // FK to User
    ...
    // }
    // fetch('api/grievance', { method: 'POST', body: JSON.stringify(payload) })
  ...
  const handleCatChange   = (e) => set('categoryId', e.target.value)
  const handlePriChange   = (e) => set('priorityId', e.target.value)
  const handleDescChange  = (e) => set('description', e.target.value)
  ...
  return (
    <div className="animate-in fade-in duration-200 max-w-2xl mx-auto lg:mx-0">
      ...
      {/* Read-only: shows who is raising (RaisedBy field) */}
      <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 mb-5 text-[13px] text-blue-800">
        <span className="font-semibold">Raised by:</span> {user?.firstName} {user?.lastName} &middot; {user?.email}
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-6 shadow-sm">
        <form onSubmit={submit} className="flex flex-col gap-5">
          <div>
            ...
            <input
              className={`w-full border rounded-md px-3 py-2 text-[13.5px] text-slate-900 focus:outline-none focus:ring-1 transition-colors ${
                errors.title
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
              ...
              <select
                className={`w-full border rounded-md px-3 py-2 text-[13.5px] focus:outline-none focus:ring-1 transition-colors ${
                  errors.categoryId
                    ? 'border-red-500 focus:border-red-500 focus:ring-red-500 text-slate-900'
                    : 'border-slate-300 focus:border-blue-600 focus:ring-blue-600 text-slate-900'
                }`}
          ...
          <div>
            ...
            <textarea
              className={`w-full border rounded-md px-3 py-2 text-[13.5px] text-slate-900 focus:outline-none focus:ring-1 transition-colors resize-y ${
                errors.description
                  ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                  : 'border-slate-300 focus:border-blue-600 focus:ring-blue-600'
              }`}
          ...
          <div>
            ...
            />
            <div className="text-[11px] text-slate-400 mt-1.5">Photos, documents, or PDFs (max 5MB).</div>
          </div>

          <div className="flex gap-2.5 pt-2">
            <button
              type="submit"
              className="inline-flex items-center justify-center px-4 py-2 bg-blue-600 text-white text-[13px] font-medium rounded-md hover:bg-blue-700 transition-colors"
            >
              Submit Grievance
            </button>
            <button
              type="button"
              onClick={goBack}
              className="inline-flex items-center justify-center px-4 py-2 bg-transparent text-slate-700 border border-slate-300 text-[13px] font-medium rounded-md hover:bg-slate-50 transition-colors"
            >
              Cancel
            </button>
          </div>
