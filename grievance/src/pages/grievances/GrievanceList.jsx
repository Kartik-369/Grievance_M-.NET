...
export default function GrievanceList({ user }) {
  ...
  const clear = () => { setFrom('2026-01-01'); setTo('2026-12-31'); setCat(''); setSid(''); setTitle(''); setStatus('all') }

  const rows = GRIEVANCES.filter(g => {
    if (cat    && g.categoryId !== +cat) return false
    if (sid    && !g.id.toLowerCase().includes(sid.toLowerCase())) return false
    if (title  && !g.title.toLowerCase().includes(title.toLowerCase())) return false
    if (status === 'pending'  && g.statusId !== 1) return false
    if (status === 'complete' && g.statusId !== 3) return false
    ...
    <div className="animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-5">
        <div>
          ...
          <div className="text-[13px] text-slate-500 mt-0.5">Kartik Balkrishna &middot; {user?.role}</div>
        </div>
        {user?.role !== 'Agent' && (
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-4 mb-4 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
          <input type="date" className="w-full border border-slate-300 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600" value={from} onChange={e => setFrom(e.target.value)} />
          <input type="date" className="w-full border border-slate-300 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600" value={to}   onChange={e => setTo(e.target.value)} />
          <select className="w-full border border-slate-300 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600" value={cat} onChange={e => setCat(e.target.value)}>
            <option value="">All Categories</option>
            {CATEGORIES.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
          <input className="w-full border border-slate-300 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600" placeholder="Grievance No." value={sid} onChange={e => setSid(e.target.value)} />
        </div>
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <input className="w-full lg:max-w-[280px] border border-slate-300 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600" placeholder="Grievance title..." value={title} onChange={e => setTitle(e.target.value)} />
            ...
            <div className="flex gap-4">
              {[['all','All'],['pending','Pending'],['complete','Complete']].map(([v, l]) => (
                <label key={v} className="flex items-center gap-1.5 text-[13px] cursor-pointer text-slate-600 hover:text-slate-900 transition-colors">
                  <input type="radio" name="status-filter" value={v} checked={status === v} onChange={() => setStatus(v)} className="accent-blue-600 w-3.5 h-3.5" />
                  ...
                  return (
                    <tr key={g.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <button onClick={() => navigate(`/grievances/${g.id}`)} className="px-2.5 py-1 bg-transparent text-blue-600 border border-blue-600 text-xs font-medium rounded hover:bg-blue-50 transition-colors">View</button>
                      </td>
                    </tr>
                  )
                })}
