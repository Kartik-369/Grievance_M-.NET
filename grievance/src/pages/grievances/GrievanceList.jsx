...
export default function GrievanceList({ user }) {
  ...
  // For demo: Student's own grievances are the first 3 (raisedBy === user.userId = 2)
  // In production these would come from api/grievance filtered by RaisedBy
  const baseList = isAdmin
    ? GRIEVANCES
    : GRIEVANCES.filter((_, i) => i < 3) // Student sees only their own
  ...
  const handleDelete = (id) => {
    // In production: DELETE api/grievance/{id}
  ...
  const handleSidChange   = (e) => setSid(e.target.value)
  const handleTitleChange = (e) => setTitle(e.target.value)
  const handleNew         = ()  => navigate('/grievances/new')
  ...
  return (
    <div className="animate-in fade-in duration-200">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-5">
        ...
        {/* Only Student can submit a new grievance */}
        {!isAdmin && (
          <button
            onClick={handleNew}
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-4 mb-4 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
          <input
            type="date"
            className="w-full border border-slate-300 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600"
            value={from}
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <input
            className="w-full lg:max-w-[280px] border border-slate-300 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600"
          ...
          <div className="flex flex-wrap items-center gap-4 w-full lg:w-auto">
            <div className="flex gap-4">
              {[['all','All'],['pending','Pending'],['complete','Complete']].map(([v, l]) => {
                const handleStatusChange = () => setStatus(v)
                return (
            ...
            <div className="flex gap-2 ml-auto lg:ml-0">
              <button className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white text-xs font-medium rounded-md hover:bg-blue-700 transition-colors">
            ...
            <table className="w-full text-left">
              ...
              <tbody className="text-[13px]">
                {rows.map((g, i) => {
                  ...
                  const viewGrievance   = () => navigate(`/grievances/${g.id}`)
                  const deleteGrievance = () => handleDelete(g.id)
                    ...
                    <tr key={g.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <button
                            onClick={viewGrievance}
                            className="px-2.5 py-1 bg-transparent text-blue-600 border border-blue-600 text-xs font-medium rounded hover:bg-blue-50 transition-colors"
                          >
                            View
                          </button>
                          {/* Admin can delete grievances via DELETE api/grievance/{id} */}
                          {isAdmin && (
                            <button
